...
class CNNVAE(nn.Module):
    ...
    def __init__(self, features,bs,H1,H2,W, device = torch.device('cuda')):
        ...
        self.fc1 = nn.Linear(self.hiddenDims[-1]*self.H1//(2**len(self.hiddenDims))*self.W//(2**len(self.hiddenDims)), 512)
        self.fc21 = nn.Linear(512, features)
        self.fc22 = nn.Linear(512, features)

        self.fc3 = nn.Linear(features, 512)
        self.fc4 = nn.Linear(512, self.hiddenDims[-1]*self.H2//(2**len(self.hiddenDims))*self.W//(2**len(self.hiddenDims)))
    ...
    def decode(self, z):
        ...
        deconv_input = deconv_input.view(self.bs, 64, 40, 80)
